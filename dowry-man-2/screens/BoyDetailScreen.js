...
export default class BoyDetailScreen extends Component {
  ...
  async addDetails(){
    if(this.state.boy_address && this.state.boy_name && this.state.boy_age && this.state.boy_number && this.state.family_details){
      let details={
        boyName:this.state.boy_name,
        boyNumber:this.state.boy_number,
        boyAge:this.state.boy_age,
        boyAddress:this.state.boy_address,
        boyFamily:this.state.family_details,
        //created_on: new Date() 
      }
    await firebase
    .database()
    .ref('/accused/'+(Math.random().toString(36).slice(2))) 
    .set(details) 
